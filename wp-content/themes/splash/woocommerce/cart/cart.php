...
<table class="shop_table shop_table_responsive cart heading-font woocommerce-cart-form__contents" cellspacing="0">
	<thead>
		<tr>
			...
			<?php endif; ?>
			<th class="product-thumbnail"><?php if(!is_layout("sccr")) esc_html_e( 'Producto', 'splash' ); ?></th>
			<th class="product-name"><?php (!is_layout("sccr")) ? esc_html_e( '', 'splash' ) : esc_html_e( 'Product', 'splash' ); ?></th>
			<th class="product-price"><?php esc_html_e( 'Precio', 'splash' ); ?></th>
			<th class="product-quantity"><?php esc_html_e( 'Cantidad', 'splash' ); ?></th>
					...
					<td class="product-thumbnail">
						<?php
							...
							if ( ! $_product->is_visible() ) {
								echo sanitize_text_field($thumbnail);
							} else {
			...
			<td colspan="6" class="actions">
				...
				<?php if ( wc_coupons_enabled() ) { ?>
					<div class="coupon">
						...
						<label for="Cupón de descuento">
							<?php esc_html_e( 'Coupon', 'splash' ); ?>:</label>
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Cupón de descuento', 'splash' ); ?>" />
							<?php if(!is_layout("bb")): ?>
								<span class="button-input-wrap <?php if(is_layout("sccr")) echo "button-gray btn-only-border"; ?>">
							<?php endif; ?>
								<input type="submit" name="apply_coupon" value="<?php esc_attr_e( 'Aplicar', 'splash' ); ?>" />
							<?php if(!is_layout("bb")): ?>
								</span>
							<?php endif; ?>
				...
				<div class="stm-actions-right">
					...
					<?php endif; ?>
						<input type="submit" class="button" name="update_cart" value="<?php esc_attr_e( 'Actualizar carrito', 'splash' ); ?>" />
					<?php if(!is_layout("bb")): ?>
						</span>
					<?php endif; ?>
					<?php if(!is_layout("sccr")) : ?>
